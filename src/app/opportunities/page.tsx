        ...
        {hasAnyData ? (
          <section className="rounded-2xl p-4 flex flex-col gap-3"
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-[14px] font-black flex-1" style={{ color: "var(--text)" }}>ملخص بياناتك</p>
              <Link href="/profile" className="text-[12px] font-black px-3 py-1.5 rounded-full"
            ...
            <div className="flex flex-wrap gap-2">
              {chips.map((c) => (
                <span key={c.label} className="px-3 py-1.5 rounded-full text-[12px] font-bold"
          ...
          <section className="rounded-3xl p-6 text-center"
            ...
            <p className="text-[16px] font-black mb-2" style={{ color: "var(--text)" }}>ما عندنا بيانات نبني عليها بعد</p>
            ...
            <Link href="/profile" className="inline-block mt-4 px-5 py-2.5 rounded-2xl font-bold text-[14px]"
              style={{ background: "var(--accent)", color: "#fff" }}>
              ← أكمل بياناتك من ملفك
            </Link>
            <p className="text-[10.5px] font-bold mt-4" style={{ color: "var(--text-muted)" }}>
              ⚖️ {OPPORTUNITIES_DISCLAIMER}
            </p>
          </section>
        )}
        ...
        {groups.map((g) => (
          <section key={g.id} className="rounded-2xl p-4 flex flex-col gap-3"
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-[14px] font-black flex-1" style={{ color: "var(--text)" }}>{g.title}</p>
            </div>
            {g.note && (
              <p className="rounded-xl px-3 py-2 text-[11.5px] font-semibold leading-relaxed"
  ...
  return (
    <div className="rounded-xl px-3.5 py-3 flex flex-col gap-1.5"
      ...
      <div className="flex items-start gap-2">
        <p className="text-[13.5px] font-black flex-1 leading-snug" style={{ color: "var(--text)" }}>{item.title}</p>
        <span className="text-[10.5px] font-black px-2 py-1 rounded-full flex-shrink-0"
      ...
      {item.score != null && item.verdict && (
        <p className="text-[13px] font-black">
          ...
          <span className="text-[11px] font-bold mr-2" style={{ color: "var(--text-muted)" }}>{item.verdict.label}</span>
        </p>
      )}
      <p className="text-[12px] leading-relaxed" style={{ color: "var(--text-muted)" }}>{item.detail}</p>
      {item.hint && (
        <p className="text-[11px] leading-relaxed" style={{ color: "var(--text-muted)" }}>💡 {item.hint}</p>
      )}
      {item.linkTo && (
        <Link href={item.linkTo} className="text-[11.5px] font-black mt-0.5"
