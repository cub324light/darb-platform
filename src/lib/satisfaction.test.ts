...
test("evaluateSatisfaction مع عتبة: أخضر/أصفر/أحمر", () => {
  ...
  assert.equal(evaluateSatisfaction(82, 85).band, "yellow");  // خلال ٥ تحت
  assert.equal(evaluateSatisfaction(70, 85).band, "red");     // أقل بكثير
});

test("evaluateSatisfaction بلا عتبة: بنود عامة ٨٥/٧٥", () => {
...
test("العتبة من الهدف الرقمي عند غياب التخصص", () => {
  ...
  assert.ok(s);
  assert.equal(s!.band, "yellow");         // 86 ضمن ٥ تحت ٩٠
