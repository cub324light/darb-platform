    ...
    return {
      group: {
        ...
        items: [{
          ...
          hint: "أضف درجاتك ووجهتك من «خطتي» (نتائجي + هدفي الجامعي)، وارجع هنا.",
          linkTo: "/plan",
        }],
      },
    };
...
function collegesGroup(
  ...
  if (target?.colleges?.length) {
    ...
    items.push({
      ...
      hint: chosen?.colleges?.length
        ...
        : "مثال — حدّد جامعتك المستهدفة من «هدفي الجامعي» في خطتي ليصير العرض أدق.",
      linkTo: "/university",
    });
  }
  ...
  if (major && major.id !== "other") {
    ...
    items.push({
      ...
      status: "قريب",
      detail: "اختر تخصصاً من «هدفي الجامعي» في خطتي ليظهر لك المطلوب له وتحليل الفجوة بدقة.",
      linkTo: "/plan",
    });
  }
