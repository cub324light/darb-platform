    ...
    return {
      group: {
        ...
        items: [{
          ...
          hint: "أضفها من ملفك (نتائجي + مستقبلي الجامعي) وارجع هنا.",
          linkTo: "/profile",
        }],
      },
    };
  ...
  if (target?.colleges?.length) {
    ...
    items.push({
      ...
      hint: chosen?.colleges?.length
        ? "جامعتك المستهدفة من ملفك — قارنها بغيرها في لوحة القبول."
        : fromTop?.colleges?.length
        ? "أعلى جامعة ملاءمةً لدرجاتك حالياً."
        : "مثال — حدّد جامعتك المستهدفة من ملفك ليصير العرض أدق.",
      linkTo: "/university",
    });
  }
  ...
  if (major && major.id !== "other") {
    ...
    if (!gap.hasData) {
      ...
      items.push({
        ...
        status: "قريب",
        detail: reqText,
        hint: "أضف نتائجك من ملفك ليتحوّل هذا إلى تحليل فجوة فعلي.",
        linkTo: "/university",
      });
    } else {
      items.push({
    ...
    items.push({
      ...
      status: "قريب",
      detail: "اختر تخصصاً من ملفك › مستقبلي الجامعي ليظهر لك المطلوب له وتحليل الفجوة بدقة.",
      linkTo: "/profile",
    });
  }
