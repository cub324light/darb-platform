...
test("arCount: مفرد/مثنّى/جمع قلّة/تمييز مفرد", () => {
  ...
  assert.equal(arCount(5, "سؤال واحد", "سؤالين", "أسئلة", "سؤالاً"), "5 أسئلة");
  assert.equal(arCount(40, "سؤال واحد", "سؤالين", "أسئلة", "سؤالاً"), "40 سؤالاً");
});
...
test("remainingSteps: يحوّل الأرقام إلى خطوات، ويحذف ما رصيده صفر", () => {
  assert.deepEqual(
    ...
    ["إنهاء وحدتين", "حل 40 سؤالاً", "مراجعة 12 خطأً"],
  );
  assert.deepEqual(remainingSteps({ remainingLessons: 0, remainingDrills: 0, unreviewedErrors: 0 }), []);
  assert.deepEqual(remainingSteps({ remainingLessons: 1, remainingDrills: 0, unreviewedErrors: 5 }), ["إنهاء وحدة واحدة", "مراجعة 5 أخطاء"]);
});
