...
export interface LessonEntity extends EntityBase {
  ...
  /* اختبر نفسك: سؤالٌ بسيط يُطرح قبل الدرس (لتحديد نقطة البداية، بلا درجات) ثم يُعاد
     في النهاية — فيرى الطالب بنفسه أنه تعلّم. إحساسٌ بالتقدّم لا تقييم. */
  diagnostic?: { prompt: string; answer: string };
  /* «أنت الآن تستطيع» — مخرجاتٌ ملموسة يُنهي بها الطالب الدرس */
  outcomes?: string[];
  /* جسرٌ سرديّ للدرس التالي (لا زرّ «التالي» فقط) — «الخطوة التالية» تُشتقّ من علاقات
