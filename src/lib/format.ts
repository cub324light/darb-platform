...
/* «ورقة/ورقتان/أوراق/ورقة» — نظيرةُ `days` لدفتر اليوم الدراسي */
export const sheets = (x: number): string =>
  x === 1 ? "ورقة واحدة" : x === 2 ? "ورقتان" : `${n(x)} ${x >= 3 && x <= 10 ? "أوراق" : "ورقة"}`;

/* «مهمة/مهمتان/مهام/مهمة» — كان يُكتب «1 مهام» في «هدف اليوم» */
export const tasks = (x: number): string =>
  x === 1 ? "مهمة واحدة" : x === 2 ? "مهمتان" : `${n(x)} ${x >= 3 && x <= 10 ? "مهام" : "مهمة"}`;
