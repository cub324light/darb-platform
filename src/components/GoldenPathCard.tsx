  ...
  return (
    <div className="rise rounded-2xl px-5 py-4"
      ...
      <div className="flex items-center justify-between gap-3 mb-2">
        <p className="text-[14px] font-bold" style={{ color: accent }}>{cardTitle}</p>
      ...
      {/* بؤرة التركيز */}
      <p className="text-[20px] font-black leading-tight" style={{ color: "var(--text)" }}>
        {state.primary.label}
      </p>
      {state.secondary && (
      ...
      {/* السبب */}
      <p className="text-[15px] leading-relaxed mt-2" style={{ color: "var(--text)" }}>
        {state.reason}
      </p>
      ...
      {state.paused.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2.5">
          {state.paused.map((p) => (
            <span key={p.label} className="text-[13px] font-semibold px-2.5 py-1 rounded-full"
      ...
      {isAdmission && state.admissionChecklist && (
        <div className="flex flex-col gap-1.5 mt-3">
          {state.admissionChecklist.map((c) => (
            <div key={c.key} className="flex items-center gap-2 text-[15px]">
      ...
      {state.englishRecommendation && (
        <p className="text-[14px] leading-relaxed mt-2.5 rounded-xl px-3 py-2"
      ...
      {milestoneText && !isAdmission && (
        <p className="text-[14px] font-semibold mt-2.5" style={{ color: "var(--text-muted)" }}>
          🔔 {milestoneText}
        </p>
      )}
      ...
      <div className="flex flex-wrap gap-2 mt-3">
        {state.suggestedActivate.length > 0 && (
          <button onClick={() => activate(state.suggestedActivate)}
            className="px-4 py-2 rounded-xl text-[15px] font-black transition active:scale-[0.97]"
        ...
        {isAdmission && (
          <Link href="/university"
            className="px-4 py-2 rounded-xl text-[15px] font-black transition active:scale-[0.97] no-underline"
            style={{ background: accent, color: "#fff" }}>
            🏛️ احسب موزونتك
          </Link>
        )}
