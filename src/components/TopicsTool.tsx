...
export default function TopicsTool({ subjects }: Props) {
  ...
  if (subjects.length === 0) {
    return (
      <p className="text-[16px] text-center py-4" style={{ color: "var(--text-muted)" }}>
  ...
  return (
    <div>
      ...
      <select value={subject} onChange={(e) => { setSubject(e.target.value); setAddedMsg(""); }}
        className="w-full rounded-2xl px-4 py-3 text-[17px] outline-none min-h-[48px] mb-3"
      ...
      {addedMsg && (
        <p className="text-[15px] text-center mt-3 font-bold" style={{ color: "var(--success)" }}>{addedMsg}</p>
      )}
    </div>
  );
}
