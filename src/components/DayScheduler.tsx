      ...
      <div className="relative w-full rounded-t-3xl max-h-[90vh] overflow-y-auto slide-up"
        ...
        <div className="sticky top-0 z-10 pt-4 pb-3 px-5" style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)" }}>
          ...
          <div className="flex items-center justify-between">
            <p className="font-black text-[19px]" style={{ color: "var(--text)" }}>{arabicDate}</p>
        ...
        <div className="px-5 py-4 pb-10">
          ...
          {tab === "manual" && (
            <div>
              {applyFeedback && (
                <div className="rounded-2xl px-4 py-3 mb-4 text-[19px] font-bold text-center"
              ...
              {dayEvents.length === 0 && !applyFeedback && (
                <p className="text-[19px] mb-4 text-center" style={{ color: "var(--text-muted)" }}>لا توجد أحداث لهذا اليوم</p>
              )}
              {dayEvents.length > 0 && (
                <div className="flex flex-col gap-2 mb-4">
                  {dayEvents.map((ev) => (
                    <div key={ev.id} className="flex items-center gap-3 rounded-2xl px-4 py-3"
                      ...
                      <span className="font-bold text-[19px] flex-1" style={{ color: "var(--text)" }}>
              ...
              {addMode === null && (
                <div className="flex gap-2 mb-4">
                  <button onClick={() => { setAddMode("study"); setAddSubject(subjects[0]?.name ?? ""); }}
                    className="flex-1 py-3 rounded-2xl font-bold text-[19px] min-h-[44px]"
              ...
              {addMode !== null && (
                <div className="rounded-2xl p-4 mb-4" style={{ background: "var(--surface2)", border: "1px solid var(--border)" }}>
                  <div className="flex items-center justify-between mb-3">
                    <p className="font-black text-[19px]" style={{ color: "var(--text)" }}>
                  ...
                  {addMode === "study" && (
                    <div className="mb-3">
                      <label className="text-[19px] font-bold mb-1 block" style={{ color: "var(--text-muted)" }}>المادة</label>
                  ...
                  {addMode === "busy" && (
                    <div className="mb-3">
                      <label className="text-[19px] font-bold mb-1 block" style={{ color: "var(--text-muted)" }}>التسمية</label>
                  ...
                  <div className="flex gap-3 mb-3">
                    <div className="flex-1">
                      <label className="text-[19px] font-bold mb-1 block" style={{ color: "var(--text-muted)" }}>من</label>
                    ...
                    <div className="flex-1">
                      <label className="text-[19px] font-bold mb-1 block" style={{ color: "var(--text-muted)" }}>إلى</label>
                  ...
                  <div className="mb-4">
                    <label className="text-[19px] font-bold mb-2 block" style={{ color: "var(--text-muted)" }}>التكرار</label>
                    <div className="flex flex-wrap gap-2">
                      {(["once", "weekly", "multi"] as const).map((r) => (
                        <button key={r} onClick={() => setRecurrence(r)}
                          className="px-3 py-2 rounded-xl text-[19px] font-bold min-h-[44px] transition"
                          style={recurrence === r
                  ...
                  <button onClick={addEvent} className="w-full py-3 rounded-2xl font-black text-[19px] min-h-[44px]"
              ...
              <div className="flex items-center gap-3 rounded-2xl px-4 py-3 mt-2"
                ...
                <span className="text-[19px] flex-1 font-bold" style={{ color: "var(--text-muted)" }}>
                  {examDate === date ? "يوم الاختبار محدد لهذا اليوم" : "تعيين كيوم اختبار"}
                </span>
                <button onClick={() => onExamDateChange(examDate === date ? null : date)}
                  className="px-3 py-2 rounded-xl text-[19px] font-bold min-h-[44px]"
                  style={examDate === date
          ...
          {tab === "ai" && !manualOnly && (
            <div>
              ...
              <textarea value={busyText} onChange={(e) => setBusyText(e.target.value)} rows={3}
                ...
                className="w-full rounded-2xl px-4 py-3 text-[19px] outline-none resize-none"
                style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", color: "var(--text)", minHeight: "90px" }} />

              {subjects.length > 0 && (
                <div className="mt-3 mb-3">
                  <p className="text-[19px] font-bold mb-2" style={{ color: "var(--text-muted)" }}>المواد:</p>
                  <div className="flex flex-wrap gap-2">
                    {subjects.map((s) => (
                      <span key={s.name} className="px-3 py-1.5 rounded-xl text-[19px] font-bold"
              ...
              {/* اختيار الاستراتيجية */}
              <div className="mb-3">
                <p className="text-[15px] font-bold mb-2" style={{ color: "var(--text-muted)" }}>استراتيجية التوزيع:</p>
                <div className="flex gap-2">
                  {([
                    ...
                    <button key={id} onClick={() => setScheduleStrategy(id)}
                      className="flex-1 py-2 rounded-xl text-[15px] font-bold transition"
              ...
              <button onClick={() => runAI()} disabled={aiLoading || !busyText.trim()}
                className="w-full py-4 rounded-2xl font-black text-[19px] min-h-[56px] mb-4 transition"
              ...
              {aiLoading && (
                <div className="flex items-center justify-center gap-3 py-6">
                  ...
                  <span className="text-[19px] font-bold" style={{ color: "var(--text-muted)" }}>دويرب يبني الخطة...</span>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="flex gap-2 mb-3">
                    <button onClick={() => { setShowEdit((v) => !v); setApplyFeedback(""); }}
                      className="flex-1 py-3 rounded-2xl font-bold text-[19px] min-h-[48px]"
                      style={{ background: "transparent", border: "1.5px solid var(--accent)", color: "var(--accent-light)" }}>
                      تعديل
                    </button>
                    <button onClick={() => { setApplyFeedback(""); applyPlan(); }}
                      className="flex-1 py-3 rounded-2xl font-black text-[19px] min-h-[48px]"
                    ...
                    <div className="flex gap-2 mt-2 mb-3">
                      {[
                        ...
                        <div key={label} className="flex-1 text-center rounded-xl py-2" style={{ background: "var(--surface2)", border: "1px solid var(--border)" }}>
                          <p className="text-[15px] font-black" style={{ color: "var(--accent-light)" }}>{val}</p>
                  ...
                  {showEdit && (
                    <div className="rounded-2xl p-4 mb-3" style={{ background: "var(--surface2)", border: "1.5px solid var(--accent)" }}>
                      <p className="text-[19px] font-bold mb-2" style={{ color: "var(--text-muted)" }}>وش تبي تعدّل؟</p>
                      <textarea value={editText} onChange={(e) => setEditText(e.target.value)} rows={2}
                        placeholder="مثال: غيّر رياضيات إلى كيمياء في الفترة الأولى..."
                        className="w-full rounded-xl px-3 py-3 text-[19px] outline-none resize-none mb-3"
                      ...
                      <button onClick={runEdit} disabled={!editText.trim()}
                        className="w-full py-2.5 rounded-xl font-bold text-[19px] min-h-[44px]"
                  ...
                  {applyFeedback && (
                    <p className="text-[19px] font-bold text-center py-2" style={{ color: applyFeedback.startsWith("أُضيف") ? "var(--success)" : "var(--danger)" }}>
                      {applyFeedback}
                    </p>
                  )}
                </div>
