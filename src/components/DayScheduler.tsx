...
function parseAISchedule(text: string, date: string, subjects: { name: string }[]): ScheduleEvent[] {
  ...
  for (const rawLine of text.split("\n")) {
    ...
    // صيغة: "من الساعة X [ص/م] إلى [الساعة] X [ص/م] [—–-] النشاط"
    const m = line.match(
      /من\s+(?:الساعة\s+)?(\d+)(?::\d+)?\s*([صم]?)\s*(?:إلى|الى)\s*(?:الساعة\s+)?(\d+)(?::\d+)?\s*([صم]?)\s*[—–\-:]\s*(.+)/
    );
    if (!m) continue;
...
اقترح جدولاً دراسياً للأوقات الفارغة.
اكتب كل فترة بهذه الصيغة الصارمة فقط:
من الساعة X [ص أو م] إلى X [ص أو م] — [المادة أو النشاط]
مثال: من الساعة 8 ص إلى 10 ص — رياضيات
لا تكتب أي نص آخر خارج هذه الصيغة.`);
  };

  const runEdit = () => {
    if (!editText.trim()) return;
    callAI(`الخطة الحالية:\n${aiResult}\n\nالتعديل المطلوب: ${editText.trim()}\n\nأعد كتابة الخطة بعد التعديل بنفس الصيغة الصارمة:\nمن الساعة X [ص أو م] إلى X [ص أو م] — [المادة أو النشاط]`);
