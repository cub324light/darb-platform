"use client";
/* أخطائي (سجلّ الأخطاء) — قسمٌ داخل وحدة «📚 أخطائي». يُستهلك مضمَّناً (embedded) في
   الصفحة الموحّدة، أو مستقلاً عبر مسار /vault. */
...
export default function ErrorsBody({ embedded = false }: { embedded?: boolean }) {
  ...
  /* ── الربط مع المحفوظات: نمنع تكرار نفس السؤال في القائمتين ──
  ...
  const convertToCard = (error: VaultError) => {
    ...
    if (cardQuestions.has(q)) {
      setCardMsg("موجودة في المحفوظات مسبقاً ✓");
    ...
    setCardMsg("أُضيفت إلى المحفوظات ✓");
    setTimeout(() => setCardMsg(null), 2200);
  };
  ...
  return (
    <div className={embedded ? "desk-wide" : "page desk-wide"}>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-14 px-4 col-span-full">
            ...
            <p className="text-[15px]" style={{ color: "var(--text-muted)" }}>
              أو راجع{" "}
              <a href="/review" className="font-bold underline" style={{ color: "var(--accent-light)" }}>المحفوظات</a>
              ...
              {isExpanded && (
                <div className="px-6 pb-6 border-t border-[var(--border)]">
                  <div className="pt-5 flex flex-col gap-4">
                    ...
                    {/* تحويل لبطاقة محفوظة — يمنع التكرار إذا كانت موجودة */}
                    {cardQuestions.has(error.question.trim()) ? (
                      <div className="w-full py-3 rounded-2xl text-[16px] font-bold text-center flex items-center justify-center gap-2"
                        ...
                        ✓ موجودة في المحفوظات — راجِعها هناك
                      </div>
                    ) : (
