"use client";
/* المحفوظات (بطاقات المراجعة، تكرار متباعد SM2) — قسمٌ داخل وحدة «📚 أخطائي».
   يُستهلك مضمَّناً (embedded) في الصفحة الموحّدة، أو مستقلاً عبر مسار /review. */
...
export default function SavedBody({ embedded = false }: { embedded?: boolean }) {
  ...
  if (mode === "session") {
    if (sessionDone) {
      ...
      return (
        <div className="min-h-[60dvh] flex flex-col items-center justify-center px-6 pb-nav relative z-[1]">
  ...
  /* ── عرض القائمة ── */
  return (
    <div className={embedded ? "relative z-[1] desk-wide" : "min-h-dvh pb-nav relative z-[1] desk-wide"}>
      {!embedded && (
        <>
          <PageGuide pageKey="review" steps={[
            { title: "المحفوظات", desc: "هنا تحفظ المعلومات اللي تبي ترسخ — قوانين، تعاريف، معادلات — على شكل بطاقات سؤال وجواب. أخطاءك من (أخطائي) ممكن تحولها لبطاقات." },
          ...
          <Dome compact>
            <div className="flex items-center justify-between">
              <h1 className="title-lg grad-title">المحفوظات</h1>
      ...
      {cards.length === 0 && (
        <div className="text-center py-12 px-6">
          <p className="text-xl font-black text-[var(--text)] mb-2">ما في محفوظات بعد</p>
