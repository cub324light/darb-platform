    ...
    <div className="rounded-3xl p-4 flex flex-col gap-3"
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[12px] font-bold" style={{ color: "var(--text-muted)" }}>التقويم الدراسي</p>
          <p className="text-[15px] font-black" style={{ color: ps.fg }}>{snap.phaseLabel}</p>
        </div>
        {snap.yearLabel && (
      ...
      <div className="grid grid-cols-2 gap-2">
        {snap.nextExam && (
          <div className="rounded-2xl px-3 py-2.5" style={{ background: "var(--surface2)" }}>
            <p className="text-[11px] font-bold" style={{ color: "var(--text-muted)" }}>أقرب اختبار</p>
            <p className="text-[13px] font-black truncate" style={{ color: "var(--text)" }}>{snap.nextExam.label}</p>
        ...
        {snap.nextSchoolFinals && snap.nextSchoolFinals.daysUntil <= 45 && (
          <div className="rounded-2xl px-3 py-2.5" style={{ background: "var(--surface2)" }}>
            <p className="text-[11px] font-bold" style={{ color: "var(--text-muted)" }}>اختبارات مدرسية</p>
            <p className="text-[13px] font-black truncate" style={{ color: "var(--text)" }}>{snap.nextSchoolFinals.label}</p>
