  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setEditing(empty())} className="px-3 py-1.5 rounded-xl text-[13px] font-bold"
          style={{ background: "var(--accent)", color: "#fff" }}>+ مصدر جديد</button>
        <button onClick={scan} disabled={busy} className="px-3 py-1.5 rounded-xl text-[13px] font-bold"
          style={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)" }}>🔍 افحص القديم</button>
        <button onClick={autoExpire} disabled={busy} className="px-3 py-1.5 rounded-xl text-[13px] font-bold"
        ...
        <select value={filter} onChange={(e) => setFilter(e.target.value as SourceCategory | "all")}
          className="px-3 py-1.5 rounded-xl text-[13px]" style={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)" }}>
      ...
      {stale.length > 0 && (
        <div className="rounded-xl p-3 text-[13px]" style={{ background: "color-mix(in srgb, var(--gold) 8%, transparent)", border: "1px solid color-mix(in srgb, var(--gold) 40%, transparent)" }}>
      ...
      {editing && (
        <div className="rounded-2xl p-4 flex flex-col gap-2" style={{ background: "var(--surface)", border: "1px solid var(--accent)" }}>
          <div className="grid grid-cols-2 gap-2">
            <input placeholder="المعرّف (id)" value={editing.id ?? ""} onChange={(e) => setEditing({ ...editing, id: e.target.value })} className="px-2 py-1.5 rounded-lg text-[13px]" style={inp} />
            <select value={editing.category} onChange={(e) => setEditing({ ...editing, category: e.target.value as SourceCategory })} className="px-2 py-1.5 rounded-lg text-[13px]" style={inp}>
              {SOURCE_CATEGORIES.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
            </select>
            <input placeholder="الاسم" value={editing.name ?? ""} onChange={(e) => setEditing({ ...editing, name: e.target.value })} className="px-2 py-1.5 rounded-lg text-[13px]" style={inp} />
            <input placeholder="الجهة" value={editing.org ?? ""} onChange={(e) => setEditing({ ...editing, org: e.target.value })} className="px-2 py-1.5 rounded-lg text-[13px]" style={inp} />
            <input placeholder="الرابط https://" value={editing.url ?? ""} onChange={(e) => setEditing({ ...editing, url: e.target.value })} className="px-2 py-1.5 rounded-lg text-[13px] col-span-2" style={inp} />
            <select value={editing.fetchMethod} onChange={(e) => setEditing({ ...editing, fetchMethod: e.target.value as FetchMethod })} className="px-2 py-1.5 rounded-lg text-[13px]" style={inp}>
              {Object.entries(FETCH_METHODS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </select>
            <select value={editing.updateMode} onChange={(e) => setEditing({ ...editing, updateMode: e.target.value as "auto" | "manual" })} className="px-2 py-1.5 rounded-lg text-[13px]" style={inp}>
            ...
            <input type="number" placeholder="الثقة 0-100" value={editing.trust ?? 90} onChange={(e) => setEditing({ ...editing, trust: Number(e.target.value) })} className="px-2 py-1.5 rounded-lg text-[13px]" style={inp} />
            <input type="number" placeholder="دورية المراجعة (يوم)" value={editing.reviewCadenceDays ?? 30} onChange={(e) => setEditing({ ...editing, reviewCadenceDays: Number(e.target.value) })} className="px-2 py-1.5 rounded-lg text-[13px]" style={inp} />
          </div>
          <div className="flex gap-2">
            <button onClick={save} disabled={busy} className="px-4 py-1.5 rounded-xl text-[13px] font-bold" style={{ background: "var(--accent)", color: "#fff" }}>حفظ</button>
            <button onClick={() => setEditing(null)} className="px-4 py-1.5 rounded-xl text-[13px]" style={{ color: "var(--text-muted)" }}>إلغاء</button>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col gap-1.5">
        {shown.map((s) => (
          <div key={s.id} className="rounded-xl px-3 py-2 flex items-center gap-2 text-[13px]"
            ...
            <button onClick={() => setEditing(s)} className="px-2 py-1 rounded-lg text-[12px]" style={{ color: "var(--accent-light)" }}>تعديل</button>
            <button onClick={() => toggle(s)} className="px-2 py-1 rounded-lg text-[12px]" style={{ color: s.status === "active" ? "var(--danger)" : "var(--success)" }}>
              {s.status === "active" ? "تعطيل" : "تفعيل"}
            </button>
          </div>
        ))}
        {shown.length === 0 && !busy && <p className="text-[13px] text-center py-4" style={{ color: "var(--text-muted)" }}>لا مصادر في هذا القسم</p>}
      </div>
    </div>
  );
}
