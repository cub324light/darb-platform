  ...
  return (
    <div className="rounded-2xl p-4" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
      <div className="flex items-center justify-between mb-2.5">
        ...
        <button onClick={onOpenGoals} className="text-[12.5px] font-bold transition active:scale-95"
      ...
      {hasCountdown ? (
        daysLeft === 0 ? (
          <p className="text-[15px] font-black" style={{ color: "var(--gold)" }}>اختبار {examTitle} اليوم! 🚀</p>
        ) : (
          <div className="flex items-baseline gap-2">
            <span className="font-mono-nums font-black text-[26px] leading-none" style={{ color: "var(--gold)" }}>
              {daysLeft.toLocaleString("ar")}
            </span>
            <span className="text-[13px] font-bold" style={{ color: "var(--text)" }}>يوم على اختبار {examTitle}</span>
          </div>
        )
      ) : (
        <p className="text-[13px]" style={{ color: "var(--text-muted)" }}>حدّد موعد اختبارك ليبدأ العدّ التنازلي.</p>
      )}

      {destination ? (
        <p className="text-[13px] font-bold mt-2 truncate" style={{ color: "var(--text)" }}>
          <span aria-hidden="true">🎓 </span>{destination}
        </p>
      ) : (
        <p className="text-[12.5px] mt-2" style={{ color: "var(--text-muted)" }}>حدّد جامعتك وتخصّصك المستهدفَين من الأهداف.</p>
      )}
    </div>
  );
}
