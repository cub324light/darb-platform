  ...
  return (
    <div className="rounded-2xl p-5 flex flex-col gap-4" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
      ...
      <div className="rounded-2xl px-4 py-3.5" style={{ background: "color-mix(in srgb, var(--accent) 8%, var(--surface2))", border: "1px solid color-mix(in srgb, var(--accent) 22%, transparent)" }}>
        <p className="text-[12px] font-bold mb-1" style={{ color: "var(--accent-light)" }}>✨ اقتباس اليوم</p>
        <p className="text-[16px] leading-relaxed font-semibold" style={{ color: "var(--text)" }}>“{quote}”</p>
      </div>
      ...
      {nextBadge && (
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[15px] font-bold flex items-center gap-1.5" style={{ color: "var(--text)" }}>
              <span>{nextBadge.icon}</span> أقرب شارة: {nextBadge.label}
            </span>
            <span className="font-mono-nums text-[14px] font-bold" style={{ color: "var(--text-muted)" }}>{nextBadge.pct}%</span>
          </div>
      ...
      {level.next && (
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="font-mono-nums text-[14px] font-bold" style={{ color: "var(--text-muted)" }}>{level.progress}%</span>
          </div>
