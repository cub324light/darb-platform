...
function Delta({ pct }: { pct: number | null }) {
  if (pct === null) return <span className="text-[13px]" style={{ color: "var(--text-muted)" }}>—</span>;
  const up = pct >= 0;
  return (
    <span className="text-[13px] font-bold" style={{ color: up ? "var(--success)" : "var(--danger)" }}>
      {up ? "▲" : "▼"} {Math.abs(pct)}%
    </span>
  );
}
...
function Tile({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="rounded-xl px-3 py-2.5" style={{ background: "var(--surface2)", border: "1px solid var(--border)" }}>
      <p className="text-[11px] font-bold mb-0.5" style={{ color: "var(--text-muted)" }}>{label}</p>
      {children}
    </div>
  );
}
  ...
  if (!hasData) {
    return (
      <div className="rounded-2xl p-6 text-center" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
        ...
        <p className="font-black text-[16px] mb-1" style={{ color: "var(--text)" }}>رؤاك تبدأ بأول جلسة</p>
    ...
    <div className="rounded-2xl p-5" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
      ...
      <div className="rounded-2xl px-4 py-4 mb-3" style={{ background: "var(--surface2)", border: `1.5px solid ${READINESS_COLOR[data.readiness.label]}` }}>
        ...
        <div className="flex items-baseline gap-2 mb-2">
          <span className="num-hero text-[28px] leading-none font-mono-nums" style={{ color: READINESS_COLOR[data.readiness.label] }}>{data.readiness.pct}%</span>
      ...
      <div className="grid grid-cols-2 gap-2">
        {data.mostActiveTime && (
          <Tile label="⏰ أنشط وقتك">
            <p className="text-[14px] font-black" style={{ color: "var(--text)" }}>{data.mostActiveTime.period}</p>
          </Tile>
        )}
        {data.mostStudied && (
          <Tile label="📈 أكثر مادة">
            <p className="text-[14px] font-black truncate" style={{ color: "var(--text)" }}>{data.mostStudied.name}</p>
          </Tile>
        )}
        {data.strong.length > 0 && (
          <Tile label="💪 أقوى مهارة">
            <p className="text-[14px] font-black truncate" style={{ color: "var(--success)" }}>{data.strong[0].name}</p>
          </Tile>
        )}
        {data.weak.length > 0 && (
          <Tile label="⚠️ تحتاج تقوية">
            <p className="text-[14px] font-black truncate" style={{ color: "var(--gold)" }}>{data.weak[0].name}</p>
          </Tile>
        )}
